...
from ..config.exceptions import ProcessingError, ConfigurationError
...
更多信息请访问: https://github.com/your-repo/integrated_script
    ...
    def _add_yolo_commands(self, subparsers):
        ...
        # 验证数据集
        validate_parser = yolo_subparsers.add_parser(
            'validate', 
            help='验证YOLO数据集'
        )
        ...
        validate_parser.add_argument(
            '--images-dir', 
            default='images', 
            help='图像目录名 (默认: images)'
        )
        validate_parser.add_argument(
            '--labels-dir', 
            default='labels', 
            help='标签目录名 (默认: labels)'
        )
    ...
    def _add_image_commands(self, subparsers):
        ...
        # 格式转换
        convert_parser = image_subparsers.add_parser(
            'convert', 
            help='转换图像格式'
        )
        ...
        # 尺寸调整
        resize_parser = image_subparsers.add_parser(
            'resize', 
            help='调整图像尺寸'
        )
        ...
        # 图像压缩
        compress_parser = image_subparsers.add_parser(
            'compress', 
            help='压缩图像'
        )
        ...
        compress_parser.add_argument(
            '--output-dir', 
            help='输出目录（默认为输入目录下的compressed子目录）'
        )
        compress_parser.add_argument(
            '--quality', 
            type=int, 
    ...
    def _add_file_commands(self, subparsers):
        ...
        # 批量复制
        copy_parser = file_subparsers.add_parser(
            'copy', 
            help='批量复制文件'
        )
        ...
        # 批量移动
        move_parser = file_subparsers.add_parser(
            'move', 
            help='批量移动文件'
        )
    ...
    def _add_dataset_commands(self, subparsers):
        ...
        # 分析数据集
        analyze_parser = dataset_subparsers.add_parser(
            'analyze', 
            help='分析数据集'
        )
    ...
    def _add_label_commands(self, subparsers):
        ...
        # 翻转标签
        flip_parser = label_subparsers.add_parser(
            'flip', 
            help='翻转标签坐标'
        )
        ...
        # 过滤标签
        filter_parser = label_subparsers.add_parser(
            'filter', 
            help='根据类别过滤标签'
        )
