...
class MenuSystem:
    ...
    def add_menu_option(
        self, option_name: str, callback: Callable, menu: Dict[str, Any] = None
    ) -> None:
        """添加菜单选项
    ...
    def remove_menu_option(self, option_name: str, menu: Dict[str, Any] = None) -> bool:
        """移除菜单选项
    ...
    def create_submenu(
        self, title: str, options: List[Tuple[str, Callable]]
    ) -> Dict[str, Any]:
        """创建子菜单
    ...
    def show_message(
        self, message: str, title: str = "信息", wait_for_input: bool = True
    ) -> None:
        """显示消息
    ...
    def show_list_selection(
        self, items: List[str], title: str = "选择", allow_multiple: bool = False
    ) -> Optional[List[int]]:
        """显示列表选择对话框
    ...
    def show_progress_menu(self, title: str, progress_callback: Callable) -> None:
        ...
        def update_progress(current: int, total: int, message: str = ""):
            ...
            if total > 0:
                ...
                print(
                    f"\r进度: |{bar}| {percentage:.1f}% ({current}/{total}) {message}",
                    end="",
                    flush=True,
                )
            else:
...
def create_simple_menu(
    title: str, options: List[Tuple[str, Callable]]
) -> Dict[str, Any]:
    """创建简单菜单的辅助函数
