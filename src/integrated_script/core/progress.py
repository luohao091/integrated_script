...
from typing import Optional, Callable, Any, Iterator, List
        ...
        def update(self, n=1):
            ...
            if self.total > 0:
                ...
                if self.current > 0:
                    ...
                    print(f"\r{self.desc}: {percent:.1f}% ({self.current}/{self.total}) ETA: {eta:.1f}s", end="")
...
class ProgressManager:
    ...
    def create_progress_bar(self, total: int, description: str = "", 
                           unit: str = "item", **kwargs) -> Optional[tqdm]:
        """创建进度条
    ...
    def update_progress(self, n: int = 1, description: str = None) -> None:
        ...
        if self.progress_bar:
            ...
            if self._total_items > 0:
                ...
                print(f"\r进度: {percent:.1f}% ({self._processed_items}/{self._total_items})", end="")
...
def progress_context(total: int, description: str = "", 
                    show_progress: bool = True, **kwargs) -> Iterator[ProgressManager]:
    """进度上下文管理器
...
def process_with_progress(items: List[Any], processor: Callable[[Any], Any],
                         description: str = "处理中", 
                         show_progress: bool = True,
                         error_handler: Optional[Callable[[Exception, Any], Any]] = None) -> List[Any]:
    """带进度显示的批量处理
