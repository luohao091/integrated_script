    ...
    def format(self, record):
        ...
        if self.use_colors:
            ...
            if ' - ' + record.levelname + ' - ' in log_message:
                log_message = log_message.replace(' - ' + record.levelname + ' - ', f' - {symbol} - ')
            return log_message
...
class LogManager:
    ...
    def _setup_root_logger(self) -> None:
        ...
        # 添加文件处理器
        log_file = self.log_dir / f"integrated_script_{datetime.now().strftime('%Y%m%d')}.log"
    ...
    def create_module_logger(self, module_name: str, 
        ...
        if log_file:
            ...
            # 模块文件格式
            module_format = "%(asctime)s - %(levelname)s - %(funcName)s:%(lineno)d - %(message)s"
...
def setup_logging(log_dir: str = "logs", log_level: str = "INFO", 
                 enable_error_file: bool = True) -> LogManager:
    """设置日志系统
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                if elapsed_time > threshold:
                    logger.warning(f"函数 {func_name} 执行时间较长: {elapsed_time:.2f}秒")
                else:
                ...
                elapsed_time = time.time() - start_time
                logger.error(f"函数 {func_name} 执行失败 (耗时 {elapsed_time:.2f}秒): {str(e)}")
                raise
