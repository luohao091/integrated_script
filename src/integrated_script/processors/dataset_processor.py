...
from ..core.utils import (
    ...
    safe_file_operation, delete_file_safe
)
from ..core.progress import progress_context, process_with_progress
from ..config.exceptions import ProcessingError, DatasetError, ValidationError
...
class DatasetProcessor(BaseProcessor):
    ...
    def analyze_dataset(self, dataset_path: str,
                       dataset_format: str = 'yolo') -> Dict[str, Any]:
        """分析数据集
    ...
    def _validate_yolo_dataset(self, dataset_dir: Path, 
                              check_integrity: bool) -> Dict[str, Any]:
        """验证YOLO格式数据集
        ...
        # 调试日志：打印孤立文件统计
        self.logger.info(f"孤立图像数量: {len(orphaned_images)}, 孤立标签数量: {len(orphaned_labels)}, 成功配对数量: {matched_pairs}")
        ...
        if orphaned_images:
            issue = {
                ...
                "preview": [str(f) for f in orphaned_images[:10]]  # 只显示前10个用于预览
            }
            result["issues"].append(issue)
            self.logger.info(f"添加孤立图像issue到结果: {issue['type']}, count={issue['count']}")
        ...
        if orphaned_labels:
            issue = {
                ...
                "preview": [str(f) for f in orphaned_labels[:10]]  # 只显示前10个用于预览
            }
            result["issues"].append(issue)
            self.logger.info(f"添加孤立标签issue到结果: {issue['type']}, count={issue['count']}")
        ...
        if check_integrity:
            ...
            if empty_labels:
                result["issues"].append({
                    ...
                    "files": [str(item["file"]) for item in empty_labels],  # 包含所有文件用于清理
                    "preview": [str(item["file"]) for item in empty_labels[:10]]  # 只显示前10个用于预览
                })
        ...
        self.logger.info(f"YOLO数据集验证完成: {result['statistics']}, success={result['success']}")
        return result
    ...
    def _detect_dataset_root(self, input_path: Path) -> Path:
        ...
        if current_path.name.lower() in ['images', 'labels']:
            ...
            if images_dir.exists() or labels_dir.exists() or classes_file.exists():
                self.logger.info(f"检测到YOLO子目录结构，使用父目录作为数据集根目录: {parent_path}")
                return parent_path
        ...
        if not (images_dir.exists() and labels_dir.exists()):
            # 检查当前目录是否直接包含图像和标签文件
            image_files = get_file_list(current_path, self.image_extensions, recursive=False)
            txt_files = get_file_list(current_path, ['.txt'], recursive=False)
            label_files = [f for f in txt_files if f.name != 'classes.txt']
            
            if image_files and label_files:
                self.logger.info(f"检测到混合目录结构（图像和标签在同一目录），将使用当前目录: {current_path}")
        
        return current_path
    
    def _validate_coco_dataset(self, dataset_dir: Path, 
                              check_integrity: bool) -> Dict[str, Any]:
        """验证COCO格式数据集"""
        # TODO: 实现COCO格式验证
        return {
            "success": False,
            "message": "COCO格式验证尚未实现"
        }
    
    def _validate_pascal_voc_dataset(self, dataset_dir: Path, 
                                    check_integrity: bool) -> Dict[str, Any]:
        """验证Pascal VOC格式数据集"""
        # TODO: 实现Pascal VOC格式验证
        return {
            "success": False,
            "message": "Pascal VOC格式验证尚未实现"
        }
    
    def _validate_csv_dataset(self, dataset_dir: Path, 
                             check_integrity: bool) -> Dict[str, Any]:
        """验证CSV格式数据集"""
        # TODO: 实现CSV格式验证
        return {
    ...
    def _analyze_yolo_dataset(self, dataset_dir: Path) -> Dict[str, Any]:
        ...
        if result["statistics"]["annotations_per_image"]:
            ...
            result["statistics"]["avg_annotations_per_image"] = sum(annotations_per_image) / len(annotations_per_image)
            result["statistics"]["max_annotations_per_image"] = max(annotations_per_image)
            result["statistics"]["min_annotations_per_image"] = min(annotations_per_image)
    ...
    def split_dataset(self, dataset_path: str, 
        ...
        try:
            ...
            self.logger.info(f"比例 - 训练: {train_ratio}, 验证: {val_ratio}, 测试: {test_ratio}")
