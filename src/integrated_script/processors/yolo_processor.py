...
from ..core.utils import (
    validate_path, get_file_list, copy_file_safe, 
    move_file_safe, delete_file_safe, create_directory
)
from ..core.progress import progress_context, process_with_progress
from ..config.exceptions import DatasetError, ValidationError, FileProcessingError
...
class YOLOProcessor(DatasetProcessor):
    ...
    def __init__(self, **kwargs):
        ...
        try:
            ...
            self.label_extension = self.get_config('yolo.label_format', self.label_extension)
    ...
    def remove_zero_only_labels(self, dataset_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            all_txt_files = get_file_list(dataset_dir, [self.label_extension], recursive=True)
            label_files = [lbl for lbl in all_txt_files if lbl.name != self.classes_file]
            ...
            with progress_context(len(zero_only_files), "删除文件") as progress:
                for label_file in zero_only_files:
                    try:
                        ...
                        # 查找并删除对应的图像文件
                        img_file = self._find_corresponding_image(label_file, dataset_dir)
    ...
    def _find_corresponding_image(self, label_file: Path, dataset_dir: Path) -> Optional[Path]:
        """查找标签文件对应的图像文件
    ...
    def get_dataset_statistics(self, dataset_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # 调用父类的验证方法
            validation_result = self._validate_yolo_dataset(dataset_dir, check_integrity=True)
    ...
    def clean_unmatched_files(self, dataset_path: str, dry_run: bool = False) -> Dict[str, Any]:
        """清理数据集中不匹配的文件
        ...
        try:
            ...
            # 调试信息：打印验证结果
            self.logger.info(f"验证结果issues数量: {len(validation_result.get('issues', []))}")
            for i, issue in enumerate(validation_result.get("issues", [])):
                self.logger.info(f"Issue {i}: type={issue.get('type')}, count={issue.get('count')}, files_count={len(issue.get('files', []))}")
            ...
            for issue in validation_result.get("issues", []):
                if issue["type"] == "orphaned_images":
                    ...
                    for invalid_label in issue["examples"]:
                        files_to_delete.append((Path(invalid_label["file"]), "invalid_labels"))
    ...
    def process_ctds_dataset(self, input_path: str, output_name: str = None) -> Dict[str, Any]:
        """处理CTDS标注数据
        ...
        try:
            ...
            return self._execute_ctds_processing(
                input_dir, project_name, obj_names_path, obj_train_data_path, 
                confirmed_type, pre_detection_result
            )
    ...
    def continue_ctds_processing(self, pre_result: Dict[str, Any], confirmed_type: str) -> Dict[str, Any]:
    ...
    def _execute_ctds_processing(self, input_dir: Path, project_name: str, 
                                obj_names_path: Path, obj_train_data_path: Path,
                                confirmed_type: str, pre_detection_result: Dict[str, Any]) -> Dict[str, Any]:
        """执行CTDS数据处理的核心逻辑
        ...
        try:
            ...
            with progress_context(len(txt_files), "处理CTDS数据") as progress:
                for txt_file in txt_files:
                    try:
                        ...
                        img_file = self._find_corresponding_ctds_image(base_name, obj_train_data_path)
                        ...
                        if img_file:
                            ...
                            result["processed_files"]["images"].append(str(new_img_path))
                        else:
                            self.logger.warning(f"未找到标签文件 {txt_file.name} 对应的图像文件")
            ...
            # 输出处理统计信息
            self.logger.info(f"CTDS数据处理完成 - 总文件数: {total_files}, 有效文件: {valid_files}, 无效文件: {invalid_count}")
            ...
            try:
                ...
                self.logger.info(f"已复制 {obj_names_path.name} 到 {classes_file_path.name}")
            except Exception as e:
    ...
    def _contains_invalid_ctds_data(self, label_file: Path, dataset_type: str = "detection") -> bool:
        """检查CTDS标签文件是否包含无效数据
            ...
            for line in lines:
                ...
                try:
                    ...
                    if dataset_type == "segmentation":
                        ...
                        if len(data) < 7:
                            self.logger.debug(f"文件 {label_file} 分割数据格式错误，需要至少7列: {line}")
                            return True
                        
                        # 检查列数是否为奇数（类别 + 偶数个坐标）
                        if len(data) % 2 == 0:
                            self.logger.debug(f"文件 {label_file} 分割数据列数应为奇数: {line}")
                            return True
                        ...
                        # 检测数据：需要5列（类别 + 4个坐标值）
                        if len(data) < 5:
                            self.logger.debug(f"文件 {label_file} 检测数据格式错误，需要至少5列: {line}")
                            return True
                    ...
                    try:
                        class_id = int(data[0])
                        if class_id < 0:
                            self.logger.debug(f"文件 {label_file} 中类别ID不能为负数: {line}")
                            return True
                    except ValueError:
                    ...
                    for i, num_str in enumerate(coord_values):
                        try:
                            ...
                            # 检查是否为负数
                            if num < 0.0:
                                self.logger.debug(f"文件 {label_file} 中存在负数坐标: {line} (第{i+2}列: {num})")
                                return True
                            # 检查是否超出范围[0,1]
                            if num > 1.0:
                                self.logger.debug(f"文件 {label_file} 中坐标超出范围[0,1]: {line} (第{i+2}列: {num})")
                                return True
                        except ValueError:
                            self.logger.debug(f"文件 {label_file} 中坐标格式错误: {line} (第{i+2}列: {num_str})")
                            return True
        ...
        try:
            ...
            for i, label_file in enumerate(sample_files):
                try:
                    ...
                    for line_num, line in enumerate(lines):
                        ...
                        if len(parts) == 5:
                            ...
                            if line_num < 3:  # 只记录前3行的详细信息
                                self.logger.info(f"文件 {label_file.name} 第 {line_num+1} 行: {len(parts)} 列 - {line[:50]}...")
                        else:
                            # 其他格式认为是分割
                    ...
                    if valid_lines == 0:
                        self.logger.warning(f"文件 {label_file.name} 没有找到有效数据行")
                        continue
                    ...
                    if detection_lines > segmentation_lines:
                        ...
                        file_type = "detection"
                        dominant_columns = 5
                        self.logger.info(f"文件 {label_file.name} 判定为检测格式 (检测行:{detection_lines}, 分割行:{segmentation_lines})")
                    else:
                        segmentation_files += 1
                        file_type = "segmentation"
                        # 取分割行中最常见的列数
                        dominant_columns = max([len(line.split()) for line in lines if line.strip() and len(line.split()) != 5], default=0)
                        self.logger.info(f"文件 {label_file.name} 判定为分割格式 (检测行:{detection_lines}, 分割行:{segmentation_lines})")
            ...
            self.logger.info(f"分析结果: 检测文件={detection_files}, 分割文件={segmentation_files}, 总计={total_analyzed}")
    ...
    def _find_corresponding_ctds_image(self, base_name: str, data_dir: Path) -> Optional[Path]:
        """查找对应的图像文件
    ...
    def merge_datasets(self, dataset_paths: List[str], output_path: str, 
                      output_name: str = None, image_prefix: str = "img") -> Dict[str, Any]:
        """合并多个YOLO数据集
        ...
        try:
            ...
            # 生成输出目录名称
            if not output_name:
                output_name = self._generate_output_name(common_classes, validated_paths)
            ...
            # 合并数据集
            merge_result = self._merge_dataset_files(validated_paths, output_dir, 
                                                   image_prefix, common_classes)
    ...
    def _validate_classes_consistency(self, dataset_paths: List[Path]) -> Dict[str, Any]:
    ...
    def _generate_output_name(self, classes: List[str], dataset_paths: List[Path]) -> str:
        """生成输出目录名称
        ...
        total_images = 0
        for dataset_path in dataset_paths:
            image_files = get_file_list(dataset_path, self.image_extensions, recursive=True)
    ...
    def _merge_dataset_files(self, dataset_paths: List[Path], output_dir: Path, 
                           image_prefix: str, classes: List[str]) -> Dict[str, Any]:
        """合并数据集文件（优化版本）
        ...
        for i, dataset_path in enumerate(dataset_paths):
            ...
            # 获取数据集文件
            image_files = get_file_list(dataset_path, self.image_extensions, recursive=True)
            label_files = get_file_list(dataset_path, [self.label_extension], recursive=True)
            ...
            dataset_result = self._merge_dataset_parallel(
                image_files, images_dir, labels_dir, 
                image_prefix, current_index, label_mapping, i+1
            )
            ...
            # 计算处理速度
            files_per_second = len(image_files) / dataset_total_time if dataset_total_time > 0 else 0
            mb_per_second = (dataset_size / (1024 * 1024)) / dataset_total_time if dataset_total_time > 0 else 0
            ...
            self.logger.info(f"数据集 {i+1} 完成: {len(image_files)} 文件, "
        ...
        performance_stats.update({
            ...
            "overall_files_per_second": round(total_images / total_time, 1) if total_time > 0 else 0,
            "overall_mb_per_second": round((performance_stats["total_size_bytes"] / (1024 * 1024)) / total_time, 1) if total_time > 0 else 0,
            "avg_file_size": round(performance_stats["total_size_bytes"] / total_images / 1024, 1) if total_images > 0 else 0  # KB
        })
        
        self.logger.info(f"合并完成! 总计: {total_images} 图像, {total_labels} 标签")
        self.logger.info(f"总耗时: {performance_stats['total_time_formatted']}")
        self.logger.info(f"平均速度: {performance_stats['overall_files_per_second']} 文件/秒, "
    ...
    def _merge_dataset_parallel(self, image_files: List[Path], images_dir: Path, 
                              labels_dir: Path, image_prefix: str, start_index: int,
                              label_mapping: Dict[str, Path], dataset_num: int) -> Dict[str, int]:
        """并行合并数据集文件
        ...
        def copy_file_batch(batch_args):
            ...
            for src_file, dst_file, file_type in copy_operations:
                try:
                    ...
                    self.logger.error(f"复制文件失败 {src_file} -> {dst_file}: {str(e)}")
                    batch_failed += 1
        ...
        self.logger.info(f"使用 {max_workers} 个线程处理 {len(batches)} 个批次，每批次 {batch_size} 个文件")
        
        # 使用进度条显示处理进度
        with progress_context(len(image_files), f"并行合并数据集 {dataset_num}") as progress:
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                # 提交批量任务
                future_to_batch = {executor.submit(copy_file_batch, batch): batch for batch in batches}
                ...
                for future in as_completed(future_to_batch):
                    try:
                        ...
                        self.logger.error(f"批次执行异常 (批次大小: {len(batch)}): {str(e)}")
                        progress.update_progress(len(batch))
        
        if failed_count > 0:
            self.logger.warning(f"数据集 {dataset_num} 有 {failed_count} 个文件复制失败")
    ...
    def _find_corresponding_label(self, image_file: Path, dataset_path: Path) -> Optional[Path]:
        """查找图像文件对应的标签文件（保留兼容性）
